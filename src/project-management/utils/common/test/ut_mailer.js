...
const { mailer, msgs, msgTemplates } = require("@utils/common");
...
describe("mailer", () => {
  describe("candidate", () => {
    ...
    it("should handle email not sent scenario and return error response", async () => {
      ...
      // Set up the response from the fake mailer
      const response = {
        accepted: [],
        rejected: [email],
      };
  ...
  describe("inquiry", () => {
    ...
    it("should send inquiry email depending on the category and return success response", async () => {
      ...
      expect(sendMailStub.firstCall.args[0].to).to.equal(email);
      expect(sendMailStub.firstCall.args[0].subject).to.equal("Welcome to STI");
      expect(sendMailStub.firstCall.args[0].html).to.equal(
  ...
  describe("verifyEmail", () => {
    ...
    beforeEach(() => {
      // Create a fake mailer object for mocking the sendMail function
      fakeTransporter = {
        sendMail: () => {},
      };
    ...
    it("should send verification email and return success response", async () => {
      ...
      const expectedMailOptions = {
        ...
        to: email,
        subject: "Verify your STI Analytics account",
      ...
      // Act
      // Assuming mailer is accessible from the verifyEmail function
      // Replace the mailer with the fakeTransporter for testing
  ...
  describe("deleteMobileAccountEmail", () => {
    ...
    it("should send delete account email with token and return success response", async () => {
      ...
      // Act
      // Assuming mailer is accessible from the deleteMobileAccountEmail function
      // Replace the mailer with the fakeTransporter for testing
  ...
  describe("newMobileAppUser", () => {
    ...
    it("should send email to new mobile app user and return success response", async () => {
      // Arrange
      const email = "johndoe@example.com";
      const subject = "Welcome to STI Mobile App";
      const message = "<p>Dear John, welcome to STI Mobile App!</p>";
      const result = {
        accepted: [email],
        rejected: [],
      };
    ...
    it("should handle email not sent scenario and return error response", async () => {
      ...
      // Act
      // Assuming mailer is accessible from the newMobileAppUser function
      // Replace the mailer with the fakeTransporter for testing
      const response = await mailer.newMobileAppUser({
        email: "johndoe@example.com",
        subject: "Welcome to STI Mobile App",
        message: "<p>Dear John, welcome to STI Mobile App!</p>",
      });
  ...
  describe("feedback", () => {
    ...
    it("should send feedback email and return success response", async () => {
      // Arrange
      const email = "johndoe@example.com";
      const subject = "Feedback on STI Analytics";
    ...
    it("should handle feedback email to automated-tests@sti.go.ug and return success response", async () => {
      // Arrange
      const email = "automated-tests@sti.go.ug";
      const subject = "Feedback on STI Analytics";
      const message = "This is a feedback message.";
      const result = {
        accepted: [],
    ...
    it("should handle email not sent scenario and return error response", async () => {
      ...
      // Act
      // Assuming mailer is accessible from the feedback function
      // Replace the mailer with the fakeTransporter for testing
      const response = await mailer.feedback({
        email: "johndoe@example.com",
        subject: "Feedback on STI Analytics",
    ...
    describe("verifyMobileEmail()", () => {
      ...
      it("should send email successfully", async () => {
        sinon.stub(mailer, "sendMail").callsFake(transporterStub);
    ...
    describe("mobileEmailVerification()", () => {
      it("should generate the email HTML content correctly", () => {
        ...
        expect(result).to.contain("Welcome to STI Analytics");
        expect(result).to.contain("Thank you for choosing STI Mobile!");
        expect(result).to.contain("Your Login Code for STI Mobile");
        expect(result).to.contain("12345");
        expect(result).to.contain(
          "You can set a permanent password anytime within your STI Analytics personal settings"
        );
      });
    });
  });
